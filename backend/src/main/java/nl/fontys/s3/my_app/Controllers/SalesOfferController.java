...
public class SalesOfferController {
    ...
    public SalesOfferController(SalesOfferRepo salesOfferRepo,
        ...
        List<SalesOfferDTO> dtos = salesOfferRepo.findAll()
                ...
                .map(so -> {
                    ...
                    List<SalesOfferLineDTO> lines = salesOfferLineRepo.findByOfferUuid(so.getUuid())
                            .stream()
                            .map(sol -> {
                                Delivery delivery = deliveryRepo.findByUuid(sol.getDeliveryUuid()).orElse(null);
                                ...
                                return new SalesOfferLineDTO(
                                        sol,
                                        new ProductDTO(
                                                productRepo.findById(sol.getProductId()).orElse(null),
                                                TEMP_SELLERDTO,
                                                new LocationDTO(true, true, new AddressDTO(fromAddress == null ? null : fromAddress))),
                                        TEMP_PRODUCTPRICEDTO,
                                        new DeliveryDTO(delivery, new AddressDTO(fromAddress == null ? null : fromAddress), new AddressDTO(toAddress == null ? null : toAddress), false));
                            })
                            .toList();
                    ...
                    List<SalesPersonDTO> people = salesOfferSalesPersonRepo.findByOfferUuid(so.getUuid())
                            .stream()
                            .map(link -> salesPersonRepo.findByUuid(link.getSalespersonUuid())) // Optional<SalesPerson>
